...
- least-privilege MV3 packaging with no remote code or network endpoint. The
  static ChatGPT entry needs only `storage`; the M2.2 Claude candidate alone
  uses the exact jointly requested optional `scripting` and
  `https://claude.ai:443/*` pair.
...
| Multiple usable composers share one Send control                       | Target-anchored semantic ownership walks the relevant region ancestors and validates all owners; ambiguity is immediately degraded and synchronously stopped without a usable context identity                                                                                       | Entirely unknown submission mechanisms remain outside the supported adapter path                    |
...
| Stale or hidden generated asset reaches a reviewed package             | Manifest-rooted graph traverses workers, scripts, pages, icons, HTML assets, and static local imports; missing, non-local, dynamic, mapped, or unallowlisted unreachable assets fail. CI runs browser tests first, then verifies and digests the same tree immediately before upload | Graph coverage is limited to declared MV3 artifact reference formats                                |
| Audit migration invents misleading contributor metadata                | M1 Audit V3 and earlier records migrate to current V4 only when catalog-owned ChatGPT identity and contributors are provable; ambiguous decisions are discarded while valid non-decision history remains                                                                             | Some legacy decision history is intentionally unavailable                                           |
| Remote code, telemetry, or data exfiltration                           | No remote dependencies at runtime, no request APIs in app source/artifact, CSP `connect-src 'none'`, URL review, and only the Claude candidate's exact optional `scripting` plus `https://claude.ai:443/*` pair; static ChatGPT remains `storage`-only                               | The ChatGPT/Claude host pages remain networked and outside extension control                        |
| Claude access removal leaves shared authority or stale registration    | Removal invalidates the permission generation synchronously, disposes and unregisters the Claude entry before dependency-aware `scripting` cleanup, and re-queries live grants; a remaining catalog-owned dynamic dependent retains `scripting`                                      | Browser cleanup can fail transiently; inactive status and startup retry prevent an activation claim |
...
| Attachment state changes after approval                                | Exact-region presence plus an opaque element-identity/mutation fingerprint is revalidated before authorization consumption and resume; scoped observation covers five classes: add, remove, replacement, attribute changes, and direct or nested text changes                        | Attachment contents are not inspected                                                               |
